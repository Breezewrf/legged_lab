...
@configclass
class AmpRecSceneCfg(InteractiveSceneCfg):
...
class ObservationsCfg:
    ...
    class PolicyCfg(ObsGroup):
        ...
        key_body_pos_b = ObsTerm(func=mdp.key_body_pos_b, params=MISSING, noise=Unoise(n_min=-0.08, n_max=0.08))
        root_height = ObsTerm(func=mdp.base_pos_z, noise=Unoise(n_min=-0.05, n_max=0.05))
    ...
    class DiscriminatorDemoCfg(ObsGroup):
        ref_root_local_rot_tan_norm = ObsTerm(func=mdp.ref_root_local_rot_tan_norm, params={"animation": MISSING, "flatten_steps_dim": False})
        ref_root_ang_vel_b = ObsTerm(func=mdp.ref_root_ang_vel_b, params={"animation": MISSING, "flatten_steps_dim": False})
        ref_joint_pos = ObsTerm(func=mdp.ref_joint_pos, params={"animation": MISSING, "flatten_steps_dim": False})
        ref_joint_vel = ObsTerm(func=mdp.ref_joint_vel, params={"animation": MISSING, "flatten_steps_dim": False})
        ref_key_body_pos_b = ObsTerm(func=mdp.ref_key_body_pos_b, params={"animation": MISSING, "flatten_steps_dim": False})
        ref_root_height = ObsTerm(func=mdp.ref_base_pos_z, params={"animation": MISSING, "flatten_steps_dim": False})
...
@configclass
class LocomotionAmpRecEnvCfg(ManagerBasedAmpEnvCfg):
    scene: AmpRecSceneCfg = AmpRecSceneCfg(num_envs=4096, env_spacing=2.5)
